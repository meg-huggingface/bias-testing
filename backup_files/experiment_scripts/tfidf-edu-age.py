...
woman_docs = map(lambda doc: doc.text, filter(lambda doc: "young" in doc.text.split(), data_reader()))
man_docs = map(lambda doc: doc.text, filter(lambda doc: "old" in doc.text.split(), data_reader()))
#print(np.asarray(vectorizer.transform(man_docs)))
tfidf_by_gender = {}
tfidf_by_gender["young"] = np.asarray(vectorizer.transform(man_docs).mean(axis=0))[0]
tfidf_by_gender["old"] = np.asarray(vectorizer.transform(woman_docs).mean(axis=0))[0]
...
pre_pandas_lines = [
    {
        ...
        "young": all_tfidf[0, w],
        "old": all_tfidf[1, w],
        "young+": all_tfidf[0, w] - all_tfidf[:, w].mean(),
        "old+": all_tfidf[1, w] - all_tfidf[:, w].mean(),
...
df.to_csv("fineweb-edu-sample-10BT-age-tfidf.csv")
