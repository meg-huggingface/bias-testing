...
nonbinary_docs = map(lambda doc: doc.text, filter(lambda doc: "non-binary" in doc.text.split(), data_reader()))
...
tfidf_by_gender["non-binary"] = np.asarray(vectorizer.transform(nonbinary_docs).mean(axis=0))[0]
...
df.to_csv("fineweb-edu-sample-10BT-gender-tfidf.csv")
